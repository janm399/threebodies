...
乍一看Scala字节码看似更简单，但是`INVOKEVIRTUAL`虚拟机的指令的内部逻辑很复杂，Java 14的“尽量避免调用`INVOKEVIRTUAL`的方式所产生的字节码虽然更长，但是执行得更快。说到这里呢，我们一般不担心低级字节码，不过我觉得了解虚拟机的作法还比较重要。
...
如果该`sealed trait`满意所有上面的条件，那么可考虑使用抽象方法，不然使用模式匹配。当然了，如果你的代码常常（通过模式匹配）将该`case class`映射另外一个值，那么为了提高重复利用性，我建议将该函数放在`case class`的`companion object`，接下来把它调用于类似于`map`的函数。
...
这样一定能够理解清楚，`NonEmpty`和`Empty`二者类型是通常类型，普通`case class`；因此代码依靠通常模式匹配的规则。将代码弯路地再重构，将`NonEmpty`和`Empty`再重构成`::`和`Nil`，我们获得下面的代码。
