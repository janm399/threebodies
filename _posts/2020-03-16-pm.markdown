...
不管你选哪一个Scala版本，基本的概念还是一摸一样。我们所表达是用户的来源有三个可能。（这里不要多考虑现实的意思。）只要我们意识到用户的数据有三个可能，就好了。以便处理这三个可能，我们必须重构调用于`users.map`的函数。

{% highlight Scala linenos %}
val users = List(Basic(1, "janmachacek"),
                 OAuth("...."),
                 Guest)
val firstUsernameCharacter = users.map { 
  case CC.Basic(_, username) => username(0)
  case CC.OAuth => 'o'
  case CC.Guest => 'g'
}
{% endhighlight %}
...
虽然这里没有任何固定的规则，但我建议只在第一个情况下将所需的方法实现于成员。因此，下面的代码表示所需的重构。
...
请注意，按照Scala句法中的《方法和成员的变量统一性》，可以使用`def`或者`val`来实现`firstUsernameCharacter`。说到这里，在什么情况下模式匹配不如避免抽象方法？
