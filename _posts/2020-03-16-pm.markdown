...
说不定你还会反对，为什么不要将`getFirstUsernameCharacter`的返回类型重构到`Option[Char]`，接下来把它在`CC`的子类实现好？我觉得这是一点点编程哲学的问题，虽然`None`所表示的是缺席，但——至少在我看来——存在着一个返回`Option`的函数表明其返回值是合理的；换句话说，如果你不接收此想法，你应该觉得下面的代码也没问题的。
