...
写完这种仪器仪表，我们可以使用示波镜来探索`app_main`中的无限循环是如何执行的，尤其探索每一个“指令”，每一个行需要都长时间。当然，我们正确得估计如`x = !x`, `light = !light`相比，调用执行I/O的函数需要的时间几数量级长。然而，这个简单方式为了了解我们遇到的问题足够了。下面的两个示波镜图所示两个信号：紫色是仪器仪表信号（即每一个无限循环重复交替到`CLK_GPIO`输入），黄色是按键输入脚信号。

![上升沿](/assets/2019-11-06-esp-idf-2/bu.png)

上面的一张示波镜图所示每450ns重复的无限循环，并且所示按键被按下时的电信号。如可以看到，以按键被按下引起的电平变低要～500ns的时间，而然在～500ns内的电平变化看似稳定。

![下降沿](/assets/2019-11-06-esp-idf-2/bd.png)

从第二张示波镜图可以看到按键被松开后引起什么样的电平曲线。如被按下时相比，被松开引起的曲线需要更长时间为了从0达到VCC电平，以致在几个重复内读取的电平可能是不决定的：注意，电平$$0-0.25*VDD$$相当于逻辑值0或者`false`，电平$$0.75*VCC-VCC$$相当于逻辑值1或者`true`，其他电平值是不一定的。

没有人可以察觉450ns这么短时间，调用一个耽误执行的函数即可。问题来了：最适合耽误的时间是多少？原来一般用户以超过～50ms的耽误为不可接受：提醒你们最低电影帧率不小于20Hz。按照这种想法，我们以调用`vTaskDelay(pdMS_TO_TICKS(50))`来把循环“速度”耽误于50ms。
...
请注意，最后一张示波镜图的间隔相当于10ms/div，除了`vTaskDelay`以外，其他源码行都还要本来的一段时间，即450ns！因此，上个示波镜图并不确切；只有我们考虑示波镜图的间隔相当于10ms，才获得正确执行安排，如下面图片所示。
