...
## 中断实现
在无限循环办理I/O不仅仅特别简明，而且我们的程序越来越复杂，就越来越难地预言无限循环的性能如何，特别是等待时间。更合适办法是利用中断的，处理器一接到中断会把目前执行的任务停止，接下来进行中断处理程序；程序进行完后，处理器会再继续进行以前停止的过程。如我们写的忙碌等待循环不同，中断是一种事件驱动编程方式。

ESP32/FreeRTOS的中断构建其实有点复杂，到时我们会探索它的细节，目前我们只来设置一个中断处理函数，让它处理在`BUTTON_GPIO`接上一个按键。硬件设计没有任何改变，为了保证稳定电平，设计还依赖内部上拉电阻器，唯一各处别是处理被按键引起的电平改变的代码。
...
为了解决这个问题，我们需要把中断处理所有的代码分开成两个函数。第一个函数还是中断处理函数，它只负责告在最短时间内诉第二个函数中断被接到了，该来进行需要更长时间的代码。好在FreeRTOS包含几个所谓synchronization primitives，其中我们来利用最简单的：semaphore。这个semaphore跟红绿灯一样的概念：有两个独立过程查看其semaphore，第一个过程等到第二个过程会把semaphore变绿色。变绿色之后，semaphore自动地再变红色，结果第一个过程再需要等到第二个过程把它变绿色。想象semaphore是一种可分享boolean变量，而其变量会告诉想把它利用的过程什么时候确实有任何可分享值。到那时候，读取其特别的变量会读取的过程停止，除非变量被其他过程被改变了，才读取的过程会执行下去。
